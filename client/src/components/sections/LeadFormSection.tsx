...
import { Sparkles, CheckCircle2, ShieldCheck, Loader2, ArrowRight, Video } from "lucide-react";
...
export default function LeadFormSection({ region }: LeadFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage("Network error connecting to Express server API.");
    } finally {
      setLoading(false);
    }
  };
                ...
                {submitted ? (
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-12 space-y-6">
                    ...
                    <p className="text-sm text-slate-600 max-w-md mx-auto leading-relaxed">
                      Thank you <span className="font-semibold text-blue-600">{formData.name}</span>. Reference ID: <span className="font-mono text-slate-900 bg-slate-100 px-2 py-0.5 rounded border border-slate-200">{leadId}</span>. Saved to Express & MongoDB backend.
                    </p>
